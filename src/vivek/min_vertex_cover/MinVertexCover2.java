...
public class MinVertexCover2 {
	...
	/**
	 * worst case time is Sum(x log x + x log x), for x = [V, 1]; 
...
//					System.out.println("removing "+node.value+" from "+connectedNode.value);
					...
					queue.downHeapify(connectedNode);  //log(Vi) in worst case, Vi is remaining nodes for ith outer loop
				}
	...
	//Sum( log Vij ), where Vi is the no. of Vertices at ith iteration of above outer loop and at jth iteration of
	...
	NodeForVersion2 findNodeToPeek(Heap queue) {	 //number of neighbours 
		...
		NodeForVersion2 tmp = node, p1=null;
