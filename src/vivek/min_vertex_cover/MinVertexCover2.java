...
public class MinVertexCover2 {
	...
	public int findMinimumVertexCover(int[][] edges, List<Integer> required, List<Integer> not_required) {
		...
		for (int i = 0; i < edges.length; i++) {
			...
			if (!temp1.track.contains(temp2)) {
				...
				// can also use stream grouping function at start for edges[][], to remove this
				// track overhead
		...
		/**
		 * a node may have increment in degree after it has been added to connected heap
		 * of previous node. so heapify() is required for connected-heaps of nodes.
		 */
	...
	/**
	 * worst case time is V * Sum(x log x + x log x), for x = [V, 1]; = V * O (1/2 *
	 * V^2 log V - 1/4 * V^2 + V ) = O ((V^2)*(log V)) = V * O ( (V^2) * (log V) ) =
	 * O(V^3 log V) in worst case Best Case = O (V^2 log V)
	 */
...
//						System.out.println("Lowest degree node="+ node.value);
			if (node.degree == 0) {
				...
				// for a given lowest degree, find that parent (P) from all candidates having
				// same degree,
				...
				// }
				// if node.degree!=1 that means there is a cycle, in that case parent will yield
				// a leaf node or
				...
				while (itr.hasNext()) {
					...
					connectedNode.connected.remove(node); // log(Vi) in worst case, Vi is remaining nodes for ith outer
															// loop
				...
				// System.out.println("polled: "+ node.value+" with deg="+node.degree+"
				// queue="+queue);
				// queue.remove(node);
			}
		}
	...
	// V * Sum( log Vij ), where Vi is the no. of Vertices at ith iteration of above
	// outer loop and at jth iteration of
	// below main loop and j = [1, Ci], where
	// Ci is the number of candidate nodes for a particular lowest degree at ith
	// iteration of above outer loop
	...
	NodeForVersion2 findNodeToPeek(Heap queue) { // number of neighbours
		...
		while (!queue.isEmpty() && node.degree == queue.peek().degree) {
			...
			// select candidate among lowest degree candidates whose parent/connected-one is
	...
	void checkForOtherMax(NodeForVersion2 node) {
		...
		while (itr.hasNext()) {
			...
			if (deg < n.degree) {
				System.out.println("\n\n----\n Found ambiguity in internal connected maxHeap for node=[" + node.value
						+ "," + node.degree + "] "

						+ "with =[" + n.value + ", " + n.degree + "]\n  as " + node + "and node.connected.peek()="
						+ node.connected.peek() + "-----\n\n");
				System.exit(1);
				break;
			}
